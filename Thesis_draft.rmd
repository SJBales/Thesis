...
Envelopes offer substantial advantages over  
...
Envelope methods begin to outperform OLS in large samples when there are many predictors with little variation. Surprisingly, there is little impact from the collinearity of the predictors.
...
Figure 4.5 shows the MSE of each technique when the variation in the predictors is low and there is little variation in the response. The advantages of LRT become more pronounced as the data become less "friendly".
...
When the data are the least "friendly"--high variation in the response and many correlated predictors with low variability--envelope methods offer large advantages over OLS. Moreover, LRT is the best dimension selection criteria in these scenarios. Also note the dramatic reduction in MSE in small sample sizes. MSE for OLS with a sample size of 25 is 15.64, whereas the MSE using LRT to select the predictor envelope resilts in a MSE of 3.23. 
...
analyzing_df %>%
  ...
  geom_bar(stat = 'identity') +
  ylab("MSE") +
  labs(title = "Figure 4.6: Advantages of Envelopes",
...
The best dimension selection criteria for normally distributed predictors is a function of the structure of the data, with the most important factor being the variation in the predictors. Low variability in the predictors makes envelopes more efficient than OLS. As seen from the above figures, LRT testing is often the best dimension selection criteria in these scenarios, and if often the best choice when the data are the most "poorly" behaved. However, LRT doesn't always have superior performance. Figure 9 shows that BIC has a lower MSE than AIC and LRT whent the data conditions aren't as severe. Thus, determining which dimension selection criteria is best for a given modeling scenario should be an iterative process. 
...
knitr::kable(table_4.2.2, "latex", caption = "Table 4.2.2: Method MSE With High and Low Variance in Variables 6 and 7")
